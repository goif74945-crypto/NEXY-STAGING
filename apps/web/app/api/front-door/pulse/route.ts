...
import { parseRunRepositoryState } from '../../../../lib/repositories/run-repository';
import { SessionRepositoryStateSchema } from '../../../../lib/repositories/session-repository';
...
const FrontDoorPulseRouteBodySchema = z
  .object({
...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    const body = FrontDoorPulseRouteBodySchema.parse(rawBody);
    ...
    const pulse = buildDashboardSummary(
      body.session_state,
      incident_state,
      body.queue_state,
      run_state,
