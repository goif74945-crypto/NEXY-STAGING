...
const RecoveryCleanRebootParamsSchema = z
  .object({
    universeId: z.string().trim().min(1).max(256),
  })
  .strict();

const RecoveryCleanRebootBodySchema = z
  .object({
    ready: z.boolean(),
...
export async function POST(
  ...
  try {
    const params = RecoveryCleanRebootParamsSchema.parse(await context.params);
    ...
    const body = RecoveryCleanRebootBodySchema.parse(rawBody);

    return NextResponse.json(
      buildSuccessEnvelope({
        universe_id: params.universeId,
        mode: 'clean_reboot' as const,
        ready: body.ready,
