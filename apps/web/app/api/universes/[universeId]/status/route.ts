...
const UniverseStatusParamsSchema = z
  .object({
    universeId: z.string().trim().min(1).max(256),
  })
  .strict();

const UniverseStatusBodySchema = z
  .object({
...
export async function POST(
  ...
  try {
    const params = UniverseStatusParamsSchema.parse(await context.params);
    ...
    const body = UniverseStatusBodySchema.parse(rawBody);
