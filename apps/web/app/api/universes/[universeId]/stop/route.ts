...
const UniverseStopParamsSchema = z
  .object({
    universeId: z.string().trim().min(1).max(256),
  })
  .strict();

const UniverseStopBodySchema = z
  .object({
...
export async function POST(
  ...
  try {
    const params = UniverseStopParamsSchema.parse(await context.params);
    ...
    const body = UniverseStopBodySchema.parse(rawBody);
